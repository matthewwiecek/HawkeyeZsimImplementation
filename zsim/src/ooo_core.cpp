...
inline void OOOCore::load(Address pc, Address addr) {
    loadAddrs[loads].addr = addr;
    loadAddrs[loads].pc = pc;
    loads++;
}
...
void OOOCore::predFalseMemOp() {
    ...
    loadAddrs[loads++].addr = -1L;
    loadAddrs[loads].pc = 0;
    loads++;
}
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
            ...
            case UOP_LOAD:
                {
                    ...
                    Address addr = loadAddrs[loadIdx].addr;
                    Address pc = loadAddrs[loadIdx].pc;
                    loadIdx++;
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
        ...
        for (uint32_t i = 0; i < 5*64/lineSize; i++) {
            uint64_t fetchLat = l1i->load(branchPc, wrongPathAddr + lineSize*i, curCycle) - curCycle;
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += lineSize) {
        ...
        //I set PC value to 0 as this is loading instructions and not executing a load Op
